...
func main1() {
	...
	result, err := search.MultiDFS(target, g, maxRecipes, tierMap)
	if err != nil {
	...
	_ = os.MkdirAll("result_multi_DFS", os.ModePerm)
	filename := strings.ReplaceAll(strings.ToLower(target), " ", "_")
	err = os.WriteFile(fmt.Sprintf("result_multi_DFS/%s_multi_bfs_level.json", filename), output, 0644)
	if err != nil {
